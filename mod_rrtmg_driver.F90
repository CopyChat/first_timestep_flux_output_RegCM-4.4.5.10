...
!
  subroutine rrtmg_driver(iyear,lout,m2r,r2a)
    ...
    if ( maxval(m2r%coszrs) > dlowval) then
      ...
      call rrtmg_sw(npr,kth,icld,lradfor,idirect, play,plev,tlay,tlev,tsfc,   &
    ...
    call rrtmg_lw(npr,kth,icld,0,lradfor,idirect, play,plev,tlay,tlev,tsfc,  &
                  h2ovmr,o3vmr,co2vmr,ch4vmr,n2ovmr,o2vmr,      &
...
!
    ...
    do k = 1 , kz
      ...
      do i = ici1 , ici2
        do j = jci1 , jci2
          o3vmr(n,k) = d_half*(o3prof(j,i,kj+1)+o3prof(j,i,kj)) * amd/amo
          n = n + 1
        end do
      end do
    end do
      ...
      do i = ici1 , ici2
        do j = jci1 , jci2
          o3vmr(n,k) = stdatm_val(calday,m2r%xlat(j,i), &
                       ...
                       stdatm_val(calday,m2r%xlat(j,i), &
                                  play(n,k),istdatm_airdn) * amd/amo
          n = n + 1
        end do
      end do
    end do
...
!     co2mmr = co2vmr*44.0D0/28.9644D0
    ...
    do k = 1 , kth
      do n = 1 , npr
        n2ovmr(n,k)   = n2ommr(n,k) * amd/44.D0
        ch4vmr(n,k)   = ch4mmr(n,k) * amd/16.D0
        co2vmr(n,k)   = cgas(2,iyear) * 1.0D-6
        o2vmr(n,k)    = 0.209460D0
        cfc11vmr(n,k) = cfc11mmr(n,k) * amd/163.1278D0
        cfc12vmr(n,k) = cfc12mmr(n,k) * amd/175.1385D0
        !
